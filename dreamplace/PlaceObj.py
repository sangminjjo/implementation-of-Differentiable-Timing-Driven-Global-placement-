...
class PlaceObj(nn.Module):
    ...
    def __init__(self, density_weight, params, placedb, data_collections,
        ...
        #for timing weight normalization
        self.wns_weight = torch.tensor(
            0.0,
            dtype = self.data_collections.pos[0].dtype,
            device = self.data_collections.pos[0].device
        )
        ...
        self.op_collections.update_density_weight_op = self.build_update_density_weight(
            params, placedb)
        #for wns_weight normalization
        #self.op_collections.update_wns_weight_op = self.build_update_wns_weight(params)
    ...
    def obj_fn(self, pos):
        ...
        @brief Compute objective.
            wirelength + density_weight * density penalty + (wns_weight * WNS)
        ...
        if len(self.placedb.regions) > 0:
            density_term = self.density_weight.dot(self.density)
        else:
            density_term = self.density * (self.density_factor * self.density_weight).item()

        self._wl_density_result = self.wirelength + density_term

        self._timing_penalty = None
        if self.params.timing_opt_flag and getattr(self.params, 'use_diff_timing', 0) == 1:
            if hasattr(self.op_collections, 'diff_timing_op'):
                iter_num = getattr(self.op_collections, 'diff_timing_iteration', 0)
                self._timing_penalty = self.op_collections.diff_timing_op(pos, iter_num)

        result = self._wl_density_result if self._timing_penalty is None else self._wl_density_result + self._timing_penalty

        logging.info(f"WL: {self.wirelength.item(): .2f} | " f"Density: {density_term.item(): .2f}")
    ...
    def obj_and_grad_fn(self, pos):
        ...
        timing_active = (
            self._timing_penalty is not None
            and self._timing_penalty.requires_grad
        )

        if timing_active:
            # WL+density backward (graph freed after this call)
            self._wl_density_result.backward()
            wl_density_grad = pos.grad.clone()
            wl_grad_norm = wl_density_grad.norm(p=1).item()

            # Timing backward (independent graph)
            pos.grad.zero_()
            self._timing_penalty.backward()
            timing_grad = pos.grad.clone()
            timing_grad_norm = timing_grad.norm(p=1).item()

            # Clip timing gradient to at most max_timing_ratio * WL grad norm
            max_ratio = getattr(self.params, 'timing_grad_max_ratio', 0.002)
            if timing_grad_norm > 1e-8:
                scale = min(1.0, max_ratio * wl_grad_norm / timing_grad_norm)
            else:
                scale = 1.0
            if scale < 1.0:
                logging.info("Timing grad clipped: scale=%.4E (timing/WL=%.4E, limit=%.4E)" %
                             (scale, timing_grad_norm / (wl_grad_norm + 1e-12), max_ratio))

            pos.grad.copy_(wl_density_grad + timing_grad * scale)
        else:
            if obj.requires_grad:
                obj.backward()

        #fixed macro가 움직이는 걸 방지
        if pos.grad is not None:
            num_nodes = self.placedb.num_nodes
            num_movable_nodes = self.placedb.num_movable_nodes
            num_filler_nodes = self.placedb.num_filler_nodes
            ...
            pos.grad[num_movable_nodes : num_physical_nodes].zero_()

            pos.grad[num_nodes + num_movable_nodes : num_nodes + num_physical_nodes].zero_()
    ...
    def check_gradient(self, pos):
        """
        @brief check gradient for debug — obj.backward() 이후에 호출됨.
               pos.grad 에 WL+density+timing 합산 gradient가 이미 있으므로
               timing forward pass를 별도로 실행하지 않음.
        """
        if not pos.requires_grad:
            return

        # pos.grad 에서 총 gradient 저장 (preconditioner 적용된 상태)
        total_grad = pos.grad.clone() if pos.grad is not None else None

        # WL, density gradient 개별 계산 (norm용)
        if pos.grad is not None:
            pos.grad.zero_()
        ...
        # timing grad = total - WL - density (별도 forward pass 불필요)
        timing_grad_norm = torch.tensor(0.0)
        iter_num = getattr(self.op_collections, 'diff_timing_iteration', 0)
        if (self.params.timing_opt_flag and getattr(self.params, 'use_diff_timing', 0) == 1
                and iter_num >= 100 and total_grad is not None):
            timing_grad = total_grad - wirelength_grad - density_grad
            timing_grad_norm = timing_grad.norm(p=1)

        logging.info("WL Force (Grad Norm)      = %.6E" % (wirelength_grad_norm))
        logging.info("Density Force (Grad Norm) = %.6E" % (density_grad_norm))
        logging.info("Timing Force (Grad Norm)  = %.6E" % (timing_grad_norm))

        # pos.grad 복원 (obj_and_grad_fn 이 return 직전에 precondition 이미 적용됨)
        if total_grad is not None:
            pos.grad.copy_(total_grad)
    ...
    '''
    #wns_weight initialization & update
    def initialize_wns_weight(self, pos, iter_num):
        """
        wirelength gradient와 timing gradient의 비율을 맞춰 wns_weight initialization
        """
        if getattr(self.params, 'use_diff_timing', 0) == 0 or not hasattr(self.op_collections, 'diff_timing_op'): 
            return
        
        with torch.enable_grad():
            # 1. Wirelength Gradient Norm 계산
            wl = self.op_collections.wirelength_op(pos)
            if pos.grad is not None: pos.grad.zero_()
            wl.backward()
            wl_grad_norm = pos.grad.norm(p=1).detach()

            # 2. Timing Gradient Norm 계산
            if pos.grad is not None: pos.grad.zero_()
            timing_penalty = self.op_collections.diff_timing_op(pos, iter_num)

            if not timing_penalty.requires_grad:
                return
            
            timing_penalty.backward()
            timing_grad_norm = pos.grad.norm(p=1).detach()
            
            # 원래 상태로 복구
            if pos.grad is not None: pos.grad.zero_()

        # [중요] 사용자가 파라미터로 지정할 WNS의 '상대적 힘' (기본값: WL 힘의 1%)
        base_timing_ratio = 0.005
        
        if timing_grad_norm > 1e-8:
            # WL Grad Norm 스케일에 맞춰 WNS Grad Norm을 정규화
            init_weight = (wl_grad_norm / timing_grad_norm) * base_timing_ratio
            self.wns_weight.data.fill_(init_weight.item())
        else:
            self.wns_weight.data.fill_(0.0)

    def build_update_wns_weight(self, params):
        """
        iteration이나 overflow 수치에 따라 wns_weight를 업데이트
        """
        start_iter = 100
        wns_step_size_inc = 1.01

        def update_wns_weight_op(iteration, overflow):
            if not (self.params.timing_opt_flag and getattr(self.params, 'use_diff_timing', 0) == 1):
                return
                # overflow가 스칼라가 아닐 수 있으므로 mean() 처리
            if iteration < 100:
                return

            # overflow가 스칼라가 아닐 수 있으므로 mean() 처리
            curr_overflow = overflow.mean().item() if isinstance(overflow, torch.Tensor) else overflow

            # 조건: Iteration이 지났거나 Overflow가 특정 수치 이하로 퍼졌을 때 WNS 적용 시작
            if iteration >= start_iter:
                
                # 1. 이제 막 조건을 만족해서 WNS weight가 0이라면 초기화(스케일 맞춤) 진행
                if self.wns_weight.item() == 0.0:
                    iter_num = getattr(self.op_collections, 'diff_timing_iteration', iteration)
                    self.initialize_wns_weight(self.data_collections.pos[0], iter_num)
                
                # 2. 이미 초기화 되었다면 weight를 조금씩 키울 수 있음 (발산에 주의)
                elif wns_step_size_inc > 1.0:
                    self.wns_weight *= wns_step_size_inc

        return update_wns_weight_op
    ...
    def build_precondition(self, params, placedb,
        ...
        @param op_collections a collection of all ops
        """
        if getattr(params, 'use_diff_timing', 0) == 1:
            # diff_timing 사용 시 preconditioning이 timing gradient를 왜곡하므로 비활성화
            logging.info("use_diff_timing=1: preconditioning disabled")
