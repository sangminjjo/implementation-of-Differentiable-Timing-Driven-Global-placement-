...
class TimingOpt(nn.Module):
    ...
    def forward(self, pos):
        ...
        """ self.wns = TimingFunction.apply(pos, self.timer, self)
        return self.wns """
    ...
    #report_tns_wns
    def report_wns(self):
        return timing_cpp.report_wns(self.timer.raw_timer)
    
""" class TimingFunction(Function):
    @staticmethod
    def forward(ctx, pos, timer, timing_opt_module):
        wns, grad_wrt_pos = timing_cpp.forward_with_grad(
            timer.raw_timer,
            pos.view(pos.numel()),
            timing_opt_module.net_names,
            timing_opt_module.pin_names,
            torch.from_numpy(timing_opt_module.flat_netpin),
            torch.from_numpy(timing_opt_module.netpin_start),
            torch.from_numpy(timing_opt_module.pin2node_map),
            torch.from_numpy(timing_opt_module.pin_offset_x),
            torch.from_numpy(timing_opt_module.pin_offset_y),
            timing_opt_module.wire_resistance_per_micron,
            timing_opt_module.wire_capacitance_per_micron,
            timing_opt_module.scale_factor,
            timing_opt_module.lef_unit,
            timing_opt_module.def_unit,
            timing_opt_module.ignore_net_degree
        )

        #save the gradient calculated by c++
        ctx.save_for_backward(grad_wrt_pos)

        return torch.tensor(wns, dtype=pos.dtype, device = pos.device)
    
    @staticmethod
    def backward(ctx, grad_output):
        grad_wrt_pos, = ctx.saved_tensors

        return grad_output * grad_wrt_pos, None, None """
