...
std::vector<std::vector<int> > _report_timing(
  ...
  for (auto& path : paths) {
    ...
    auto& pl = result.back();
    for (auto& point : path) {
      auto it = pin_name2id_map.find(point.pin.name());
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("report_wns", &DREAMPLACE_NAMESPACE::TimingCpp::report_wns, "report WNS as tensors");
  ...
  m.def("extract_flute_topology", &DREAMPLACE_NAMESPACE::TimingCpp::extract_flute_topology, "Extract FLUTE Steiner Tree topology for Differentiable STA");
}
