...
class PlaceDB (object):
    ...
    def load_instance_celltype(self, file):
        self.instance_celltype = {}

        # 1. node_name2id_map의 키를 깔끔한 문자열로 변환한 안전한 맵 생성
        safe_node_map = {}
        for k, v in self.node_name2id_map.items():
            key_str = k.decode('utf-8') if isinstance(k, bytes) else str(k)
            # 숨겨진 문자나 백슬래시, 따옴표 모두 제거
            key_str = key_str.replace('\x00', '').replace('\\', '').replace('"', '').replace("'", '').strip()
            key_str = key_str.split('.DREAMPlace')[0]
            safe_node_map[key_str] = int(v)
        #debug
        with open("debug/node_name2id_map.json", "w") as f:
            json.dump(safe_node_map, f, indent=2)
        ...
        with open(file) as f:
            for line in f:
                ...
                # 파일에서 읽은 이름도 완벽히 정제
                inst = tokens[0].replace('\x00', '').replace('\\', '').replace('"', '').replace("'", '').strip()
                cell = tokens[1].replace('\x00', '').replace('\\', '').replace('"', '').replace("'", '').strip()
                
                # 정제된 맵에서 찾기
                if inst in safe_node_map:
                    node_id = safe_node_map[inst]
                    self.instance_celltype[node_id] = cell  # 키: 정수(node_id), 값: 문자열(cell)
