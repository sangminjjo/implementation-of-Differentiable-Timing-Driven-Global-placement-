...
from dreamplace.ops.diff_timing.diff_timing_loss import DifferentiableTimingLoss
import json
...
class NonLinearPlace(BasicPlace.BasicPlace):
    ...
    #=========================================
    #modified for differentiable TDP

        if params.timing_opt_flag and getattr(params, 'use_diff_timing', 0) ==1 :
            logging.info("Initializaing Differentiable Timing Engine...")

            placedb.load_instance_celltype("instance_celltype.txt")
            
            self.extracted_timing_arcs, self.extracted_constraint_arcs, self.endpoint_list = self._extract_timing_graph(placedb, timer) #_extract_timing_graph 구현해야됨
            self.op_collections.diff_timing_op = DifferentiableTimingLoss(
                num_pins = placedb.num_pins,
                timing_arcs = self.extracted_timing_arcs,
                constraint_arcs = self.extracted_constraint_arcs,
    ...
    def _extract_timing_graph(self, placedb, timer):
        """ opentimer로부터 timing arc 정보를 추출, cell / net arc 분류해 추출
            constraint arc(FF clock to FF D), endpoint(FF D pin, PO pins)"""
        os.makedirs("debug", exist_ok=True)
        cache_file = "debug/timing_arcs.pkl"

        # Skip extraction if cache exists
        if os.path.exists(cache_file):
            logging.info("_extract_timing_graph: cache found at %s, skipping extraction.", cache_file)
            with open(cache_file, "rb") as f:
                return pickle.load(f)
            

        logging.info("Extracting timing graph from OpenTimer...")
        t_start = time.time()

        fast_pin_map = {}
        for k, v in placedb.pin_name2id_map.items():
            if isinstance(k, bytes):
                fast_pin_map[k.decode('utf-8')] = v
            else:
                fast_pin_map[k] = v

        if not os.path.exists("debug/fast_pin_map.json"):
            with open("debug/fast_pin_map.json", "w") as f:
                json.dump(fast_pin_map, f, indent=2)

        src_list = []
        dst_list = []
        constraint_arc_src_list = []
        constraint_arc_dst_list = []

        endpoint_set = set()

        graph_file = "temp_timing_graph.txt"
        if not os.path.exists(graph_file):
            timer.dump_graph(graph_file)
            t_dump = time.time()
            logging.info(f" -> dump_graph file generated in {t_dump - t_start: .2f} seconds. ")
        else:
            logging.info(" -> reusing existing %s", graph_file)
            t_dump = time.time()
        ...
        with open(graph_file, "r") as f:
            for line in f:
                ...
                if "->" in line_stripped:
                    ...
                    if len(tokens) >= 3:
                        src_name = tokens[0].strip('";')
                        dst_name = tokens[2].strip('";')

                        src_pin = src_name.split(':')[-1].lower()
                        dst_pin = dst_name.split(':')[-1].lower()

                        is_clk_src = src_pin == "ck" or src_pin == "clk" or src_pin == "iccad_clk" or src_pin.startswith("lcb")
                        is_data_dst = dst_pin == "d"
                        # is_lcb = src_pin.startswith("lcb") or dst_pin.startswith("lcb")

                        #FF의 clock to D -> constraint arc로
                        if is_clk_src and is_data_dst:
                            constraint_src_id = fast_pin_map.get(src_name)
                            constraint_dst_id = fast_pin_map.get(dst_name)
                            if constraint_src_id is not None and constraint_dst_id is not None:
                                constraint_arc_src_list.append(constraint_src_id)
                                constraint_arc_dst_list.append(constraint_dst_id)
                                endpoint_set.add(constraint_dst_id) #FF의 D pin을 endpoint로

                            continue


                        #local clock buffer의 cell arc -> constraint arc로
                        # if is_lcb:
                        #     constraint_src_id = fast_pin_map.get(src_name)
                        #     constraint_dst_id = fast_pin_map.get(dst_name)
                        #     if constraint_src_id is not None and constraint_dst_id is not None:
                        ...
                        src_id = fast_pin_map.get(src_name)
                        dst_id = fast_pin_map.get(dst_name)
                ...
                elif line_stripped.startswith("//PO:"):
                        node_name = line_stripped.split('"')[1]
                        po_id = fast_pin_map.get(node_name)
                        if po_id is not None:
                            endpoint_set.add(po_id)
                            continue



        timing_arcs = list(set(zip(src_list, dst_list)))
        constraint_arcs = list(set(zip(constraint_arc_src_list, constraint_arc_dst_list)))
        endpoint_list = list(endpoint_set)



        t_parse = time.time()
        logging.info(f" -> File parsed into raw lists in {t_parse - t_dump:.2f} seconds.")

        src_tensor = torch.tensor(src_list, dtype=torch.long)
        dst_tensor = torch.tensor(dst_list, dtype=torch.long)

        cst_src_tensor = torch.tensor(constraint_arc_src_list, dtype=torch.long)
        cst_dst_tensor = torch.tensor(constraint_arc_dst_list, dtype=torch.long)

        pin2node = torch.tensor(placedb.pin2node_map, dtype=torch.long)
        pin2net = torch.tensor(placedb.pin2net_map, dtype=torch.long)

        cst_pin2node = torch.tensor(placedb.pin2node_map, dtype=torch.long)
        cst_pin2net = torch.tensor(placedb.pin2net_map, dtype=torch.long)

        src_nodes = pin2node[src_tensor]
        dst_nodes = pin2node[dst_tensor]
        src_nets = pin2net[src_tensor]
        dst_nets = pin2net[dst_tensor]

        cst_src_nodes= pin2node[cst_src_tensor]
        cst_dst_nodes = pin2node[cst_dst_tensor]
        cst_src_nets = pin2net[cst_src_tensor]
        cst_dst_nets = pin2net[cst_dst_tensor]


        for fname, data in [
            ("debug/pin2node.json", pin2node),
            ("debug/pin2net.json", pin2net),
            ("debug/src_nodes.json", src_nodes),
            ("debug/dst_nodes.json", dst_nodes),
            ("debug/src_nets.json", src_nets),
            ("debug/dst_nets.json", dst_nets),
            ("debug/cst_src_nodes.json", cst_src_nodes),
            ("debug/cst_dst_nodes.json", cst_dst_nodes),
            ("debug/cst_src_nets.json", cst_src_nets),
            ("debug/cst_dst_nets.json", cst_dst_nets),
            ("debug/cst_pin2node.json", cst_pin2node),
            ("debug/cst_pin2net.json", cst_pin2net),
        ]:
        ...
        with open(cache_file, "wb") as f:
            pickle.dump((timing_arcs, constraint_arcs, endpoint_list), f)
        logging.info("_extract_timing_graph: cached timing_arcs & constraint_arcs to %s", cache_file)

        return timing_arcs, constraint_arcs, endpoint_list
        ...
        if params.global_place_flag:
            ...
            for cur_stage, global_place_params in enumerate(global_place_stages):
                ...
                def one_descent_step(
                    ...
                    t0 = time.time()
                    if hasattr(self.op_collections, 'diff_timing_op'):
                        self.op_collections.diff_timing_iteration = iteration
                    ...
                    logging.info("optimizer step %.3f ms" % ((time.time() - t3) * 1000))
                      ...
                      # Perform timing-opt. diff_timing과 충돌 방지를 위해 use_diff_timing이 1일 때는 net weighting 실행 안함
                    if params.global_place_flag and params.timing_opt_flag and \
                        getattr(params, 'use_diff_timing', 1) == 1 and \
                        iteration >= 200 and iteration % 20 == 0:
                        ...
                        if params.timer_engine == "opentimer":
                            ...
                            with torch.no_grad():
                                cur_loss = model.obj_fn(self.pos[0]).item()

                            logging.info("loss: %f", cur_loss)
                ...
                max_global_iteration = params.max_global_iteration


                for Lgamma_step in range(model.Lgamma_iteration):
                    if iteration > max_global_iteration:
                        break
                    ...
                    for Llambda_density_weight_step in range(model.Llambda_density_weight_iteration):
                        ...
                        if Llambda_flat_iteration > 1:
                            ...
                            # wns_weight update 추가
                            # if hasattr(model.op_collections, 'update_wns_weight_op'):
                            #     model.op_collections.update_wns_weight_op(
                            #         iteration,
