...
def place(params, learning_rate_value):
    ...
    if params.timing_opt_flag:
        tt = time.time()
        # Get timer engine from parameters,default to "opentimer"
    ...
    #save critical path information
    ...
    # 2. Placement 솔루션을 파일로 씁니다. (여기서 한 번만 쓰면 됩니다)
    placedb.write(params, gp_out_file)

    # 3. 작성된 파일을 바탕으로 Critical path 정보를 저장/리포트 합니다.
    if params.timing_opt_flag and timer is not None:
        logging.info("Dumping top critical paths...") # 오타(Dumpin -> Dumping)도 수정
        # placedb.write는 위에서 이미 했으므로 생략합니다.
        
        timer.update_timing()
        paths = timer.report_timing(20)

        with open("results/critical_path.txt", "w") as f:
            for p in paths:
                f.write(str(p) + "\n")
