...
typedef struct STATimingECOContext STATimingECOContext;
...
typedef struct CellFootprints {
  /**
   * The total number of cell function keys.
   */
  uintptr_t num_cell_function_keys;
  /**
   * A Pointer to an array of C strings representing function keys.
   *  (size: `num_cell_function_keys`)
   */
  const char *const *cell_function_keys;
  /**
   * A Pointer to an array of C strings representing footprint names.
   *  (size: `cell_st[num_cell_function_keys]`)
   */
   ...
   *  (size: `cell_st[num_cell_function_keys] + 1`)
   */
  const uintptr_t *cell_pin_st;
  /**
   * A Pointer to an array of C strings representing physical variants of footprints.
   * (size: `cell_physical_variant_st[cell_st[num_cell_function_keys]]`)
   */
  const char *const *cell_physical_variants;
  /**
   * Pointer to an array of starting indices for each footprint's physical variants in the flattened arrays.
   * (size: `cell_st[num_cell_function_keys] + 1`)
   */
  const uintptr_t *cell_physical_variant_st;
} CellFootprints;
...
typedef struct VecCSRCppInterface {
  uintptr_t num_st;
  /**
   * flattened list start, length is `num_st + 1` elements
   */
  uintptr_t *start;
  /**
   * flattened list, indexed by `start`, length is `start[num_st]` elements
   */
  uintptr_t *items;
} VecCSRCppInterface;
...
/**
 * Returns the number of available CUDA devices detected by the library.
 *
 * Returns 0 if the library was not compiled with the "cuda" feature.
 */
uintptr_t heterosta_get_num_cuda_devices(void);
...
/**
 * Create a new heterosta instance on heap with a specified CUDA device ID,
 * and return its opaque pointer.
 *
 * The pointer should be freed using [`heterosta_free`] afterwards.
 */
struct STAHoldings *heterosta_new_with_device(uint8_t cuda_device_id);

/**
 * Set the CUDA device ID for a given heterosta instance.
 *
 * # Arguments
 *
 * * `sta` - A mutable pointer to the STAHoldings instance.
 * * `cuda_device_id` - The ID of the GPU device to be used.
 */
void heterosta_set_cuda_device_id(struct STAHoldings *sta, uint8_t cuda_device_id);
...
/**
 * Flatten all internal design databases with footprint prefer.
 *
...
/**
 * Reads multiple SDC constraints files.
 *
 * - `paths`: An array of C string pointers, each representing an SDC file path.
 * - `num_sdc`: The number of paths in the array.
 *
 * All files will be parsed sequentially.
 * Returns true if at least one file is successfully parsed, false if all fail.
 */
bool heterosta_batch_read_sdc(struct STAHoldings *sta, const char *const *paths, uintptr_t num_sdc);
...
/**
 * Report worst negative slack (WNS) and total negative slack (TNS) for max(setup) checks.
 *
 ...
 */
bool heterosta_report_wns_tns_max(const struct STAHoldings *sta,
                                  float *wns,
                                  float *tns,
                                  bool use_cuda);

/**
 * Report worst negative slack (WNS) and total negative slack (TNS) for min(hold) checks.
 *
 * The function must have been called after `heterosta_update_arrivals` and `heterosta_update_delay`.
 */
bool heterosta_report_wns_tns_min(const struct STAHoldings *sta,
                                  float *wns,
                                  float *tns,
                                  bool use_cuda);
 ...
 */
void heterosta_dump_paths_max_to_file(const struct STAHoldings *sta,
                                      uintptr_t num_paths,
                                      uintptr_t nworst,
...
/**
 * Print the timing report of the first num hold timing paths to the file.
 *
 * ## Parameters
 * - `sta`: A reference to the `STAHoldings` structure.
 * - `num_paths`: The number of paths to dump.
 ...
 * - `use_cuda`: A boolean indicating whether to use CUDA for processing.
 */
void heterosta_dump_paths_min_to_file(const struct STAHoldings *sta,
                                      uintptr_t num_paths,
                                      uintptr_t nworst,
                                      const char *file_path,
                                      bool use_cuda);
 ...
 * - `result_use_cuda`: A boolean flag indicating whether results should reside on GPU or CPU.
 *
 * ## Returns
 ...
 * ## Usage Example (in C++)
 * ```cpp
 * PBAPathCollectionCppInterface* path_interface = heterosta_report_paths_setup(sta, num_paths, nworst, slack_lesser_than, use_cuda, result_use_cuda);
 ...
 *
 * ## Notes:
 * - When `result_use_cuda` is `true`, the data will initially reside on the GPU.
...
/**
 * Get the function key of a cell based on its type.
 *
 * This function retrieves the function key (logical representation) of a cell
 * based on the provided cell type and timing context (early or late).
 * Parameters:
 * - sta: Reference to the STAHoldings object containing cell data.
 * - celltype: C string representing the cell type.
 * - early_or_late: Timing context (0: early, 1: late).
 * Returns:
 * - A pointer to a C string containing the function key, or null if the cell
 * type is not found.
 */
const char *heterosta_get_function_key_by_celltype(const struct STAHoldings *sta,
                                                   const char *celltype,
                                                   TimerEarlyOrLate early_or_late);
 ...
 */
const char *const *heterosta_get_celltypes_by_function_key(const struct STAHoldings *sta,
 ...
 * returned by `heterosta_get_celltypes_by_function_key`.
 *
 * Parameters:
