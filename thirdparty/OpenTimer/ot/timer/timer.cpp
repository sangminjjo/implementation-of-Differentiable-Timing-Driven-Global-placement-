...
void Timer::export_instance_celltype(const std::string& file) const {

  std::ofstream ofs(file);

  if(!ofs.is_open()) {
    OT_LOGE("Failed to open file for export: ", file);
    return;
  }

  for(const auto& [inst_name, gate] : _gates) {
    std::string cell_type = "";
    
    // MIN(early) 라이브러리가 있으면 그걸 쓰고, 없으면 MAX(late) 라이브러리 사용
    if (gate._cell[MIN]) {
        cell_type = gate._cell[MIN]->name;
    } else if (gate._cell[MAX]) {
        cell_type = gate._cell[MAX]->name;
    }
    
    // 셀 타입이 정상적으로 찾아졌을 때만 파일에 기록
    if (!cell_type.empty()) {
        ofs << inst_name << " " << cell_type << "\n";
    }
  }
}
